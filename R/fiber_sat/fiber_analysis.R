  ...
  model1 <- lm(fiber$X1 ~ fiber$X2 + fiber$X4);
  pairs(fiber)
...
for(i in 1:n){ if(abs(std[i])> 1.5)text(model1$fitted.value[i]+indent,std[i],i,cex=0.7)}
...
ext_env(fiber$X1,fiber$X2,fiber$X4,1000)

source("R/fiber_sat/dffits.R")
dffit_sum(fiber$X1,fiber$X2,fiber$X4,1000)

#################################Deletion##################

fiber_new <- fiber[-c(8,18,19,15),]
model2 <- lm(fiber_new$X1 ~ fiber_new$X2 + fiber_new$X4);

pairs(fiber_new)
p <- length(model2$coefficients);
n <- length(fiber_new$X1);
...
for(i in 1:n){ if(abs(std[i])> 1.5)text(model2$fitted.value[i]+indent,std[i],i,cex=0.7)}
...
source("R/fiber_sat/ext_env.R")
ext_env(fiber_new$X1,fiber_new$X2,fiber_new$X4,1000)
